      ...
      <div style={styles.grid}>
        {Object.values(platforms).map(platform => {
          ...
          return (
            ...
            >
              <span style={{ fontWeight: isSelected ? '600' : '400' }}>{platform.name}</span>
            </button>
          );
        })}
      </div>
...
const styles = {
  container: { padding: '24px 32px', borderBottom: '1px solid var(--border-color)' },
  title: { margin: '0 0 16px 0', fontSize: '1.3rem', fontWeight: '600', color: 'var(--text-main)' },
  grid: { display: 'flex', gap: '16px', flexWrap: 'wrap' },
  button: { display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '12px 24px', borderRadius: '30px', border: '1px solid', transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)', outline: 'none', fontFamily: 'var(--font-body)', fontSize: '1.15rem' }
};
